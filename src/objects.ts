...
export function isValid(question: Question, answer: string): boolean {
    ...
    else if (question.type === "multiple_choice_question") { return Boolean(question.options.find((option: string): boolean => option === answer)); }
    return false;
}
...
export function toMarkdown(question: Question): string {
    if (question.type === "multiple_choice_question") { return "# " + question.name + "\n" + question.body + "\n" + question.options.map((option: string): string => "- " + option).join("\n"); }
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newQuestion = {...oldQuestion, name: "Copy of " + oldQuestion.name, published: false, id: id};
    return newQuestion;
}
...
export function addOption(question: Question, newOption: string): Question {
    const newOptionsList = [...question.options, newOption];
